...
public:
  ...
  AdaptiveSlidingModeControl()
  {
    ...
    static const float dkc_x = 1.0;
    static const float dkc_y = 1.0;
    static const float dgamma_x = 1.0;
    static const float dgamma_y = 1.0;
    static const float dlambda_x = 1.0;
    static const float dlambda_y = 1.0;

    n.param("/tracking_control_sast_2022/kc_x", kc_x, dkc_x);
    n.param("/tracking_control_sast_2022/kc_y", kc_y, dkc_y);
    n.param("/tracking_control_sast_2022/gamma_x", gamma_x, dgamma_x);
    n.param("/tracking_control_sast_2022/gamma_y", gamma_y, dgamma_y);
    n.param("/tracking_control_sast_2022/lambda_x", lambda_x, dlambda_x);
    n.param("/tracking_control_sast_2022/lambda_y", lambda_y, dlambda_y);
  ...
  void control()
  {
    if (testing == 1 && arduino == 1){
      ...
      L_dot_x = (pow(std::abs(kc_x),0.5) * pow(std::abs(s_x),0.5)) - (pow(std::abs(gamma_x),0.5) * L_x*L_x);
      
      L_x = (integral_step)*(L_dot_x + L_dot_last_x)/2 + L_x; //integral to get the x adaptative gain
      L_dot_last_x = L_dot_x;

      L_dot_y = (pow(std::abs(kc_y),0.5) * pow(std::abs(s_y),0.5)) - (pow(std::abs(gamma_y),0.5) * L_y*L_y);
...
{
  ros::init(argc, argv, "tracking_control_sast_2022");
