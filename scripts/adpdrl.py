...
NODE_NAME_THIS = 'adpdrl'

model = rospy.get_param("adpdrl/model", "exp50_example6000")
...
#Thruster data publishers
    ...
    def control(self,u_desired,psi_desired):
        ...
        state[0] -= psi_desired
        if (math.fabs(state[0]) > (np.pi)):
            state[0] = (state[0]/math.fabs(state[0]))*(math.fabs(state[0])-2*np.pi)
...
def main():
    ...
    rl_control.load_weights('/home/alex/Documents/rasp_ws/src/sensors/scripts/weights/speed_heading/'+ model + '.npz')
