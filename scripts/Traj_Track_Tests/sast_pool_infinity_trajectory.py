...
def main():
    rospy.init_node('sast_pool_infinity_trajectory', anonymous=False)
    rate = rospy.Rate(100)
    t = Test()
    ...
    yddot = yddot['data']
    xdddot = sio.loadmat(dir_name + '/mat/traj2/pool_xdddot.mat')
    xdddot = xdddot['data']
    ydddot = sio.loadmat(dir_name + '/mat/traj2/pool_ydddot.mat')
    ydddot = ydddot['data']
    time.sleep(10)
    rospy.logwarn("Start")
    if t.testing:
