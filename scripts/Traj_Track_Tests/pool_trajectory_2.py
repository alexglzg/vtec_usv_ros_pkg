...
trajectory = rospy.get_param("pool_trajectory_2/trajectory", 'pool_trajectory_2')
...
def main():
    rospy.init_node('pool_trajectory_2', anonymous=False)
    rate = rospy.Rate(100)
    t = Test()
    ...
    yddot = yddot['data']
    xdddot = sio.loadmat(dir_name + '/mat/' + trajectory + '/pool_xdddot.mat')
    xdddot = xdddot['data']
    ydddot = sio.loadmat(dir_name + '/mat/' + trajectory + '/pool_ydddot.mat')
    ydddot = ydddot['data']
    time.sleep(10)
    rospy.logwarn("Start")
    if t.testing:
